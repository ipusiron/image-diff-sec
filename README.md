...
**🆕 異なるサイズの画像比較機能**をテストするには、`qr-test-generator.html`を使用してください：

1. ブラウザで`qr-test-generator.html`を開く
2. 3つのテストシナリオから画像をダウンロード：
   - **シナリオ1**: フルサイズ(400×400px) vs トリミング版(200×200px)
   - **シナリオ2**: 大きな背景(500×300px) vs 小さな背景(300×200px)  
   - **シナリオ3**: 正常版(350×350px) vs 改ざん版(250×400px)
3. 生成された画像をImageDiffSecで比較テスト

#### 期待される結果

- **シナリオ1**: QRコード部分が完全一致として検出
- **シナリオ2**: 同一QRコードが自動検出され、差分率0%
- **シナリオ3**: URLの違いによる差分が検出される
